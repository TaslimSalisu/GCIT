...
public class AssignmentOne {
	...
	private static void pileGame() {
		...
		System.out.println("Player one's name is " + playerOne.getName() + " and player two's name is " + playerTwo.getName());
	...
	private static void gameStart(int numOfChips, Player one, Player two) {
		...
		chipsAllowed = numOfChips % 2 == 0 ? numOfChips / 2: (numOfChips - 1) / 2;
		while(numOfChips > 0 ) {
			if (numOfChips == 1) {
				System.out.print(one.getName() + " There is 1 chip remaining. You can only take 1");
				chipsRemoved = scanner.nextInt();
				while (chipsRemoved != 1) {
					System.out.print("You must remove 1 chip");
					chipsRemoved = scanner.nextInt();
				}
				one.addToChips(1);
				break;
			...
			System.out.println(one.getName() + " has " + one.getChips() + " chips. \n" + two.getName() + " has " + two.getChips() + " chips");
			System.out.println("Your turn " + one.getName());
			System.out.println(one.getName() + " please remove some chips. You're allowed to remove 1 to " + chipsAllowed);
			chipsRemoved = scanner.nextInt();
			while (chipsRemoved < 1 || chipsRemoved > chipsAllowed) {
				System.out.println("Wrong number of chips removed. You're allowed to remove 1 to " + chipsAllowed);
				chipsRemoved = scanner.nextInt();
			}
			...
			if (numOfChips == 1) {
				System.out.println(two.getName() + " There is 1 chip remaining. You can only take 1");
				chipsRemoved = scanner.nextInt();
				while (chipsRemoved != 1) {
			...
			System.out.println(two.getName() + " please remove some chips. You're allowed to remove 1 to " + chipsAllowed);
			chipsRemoved = scanner.nextInt();
			while (chipsRemoved < 1 || chipsRemoved > chipsAllowed) {
				System.out.println("Wrong number of chips removed. You're allowed to remove 1 to " + chipsAllowed);
				chipsRemoved = scanner.nextInt();
	 ...
	 */
	private static void numberGuessing(int randomNumber, int allowedGuesses) {
